...
from domain import ccs_chain as CCS
...
OUTPUT_DIR = Path(__file__).resolve().parents[1] / 'outputs' / 'legacy'
df.to_html(OUTPUT_DIR / 'results_preview.html', index=False)
df.to_excel(OUTPUT_DIR / 'results_preview.xlsx', index=False)
